...
class Syosetu(BaseSite):

    def __init__(self, session: ClientSession):
        super().__init__(session)
        self.site: str = "syosetu"
        self.domain: str = read_config("domain")["syosetu"]
        # 容易触发反爬强制单线程
    ...
    async def valid_cookie(self) -> bool:
        ...
        if res:
            log.info("syosetu校验cookie成功")
            return True
        log.info("syosetu cookie失效")
        return False

    async def login(self):
        cookie = Cookie()
        cookie.source = "syosetu"
        if not read_config("login_info")["syosetu"]["cookie"]:
            raise Exception("syosetu未配置登录信息")
        cookie.cookie = read_config("login_info")["syosetu"]["cookie"]
        # 再校验一次cookie
        self.cookie = cookie
        self.header["Cookie"] = cookie.cookie
        is_effective_cookie = await self.valid_cookie()
        if not is_effective_cookie:
            raise Exception("syosetu登录失败 cookie失效")
        # 数据库更新
    ...
    async def get_book_list(self):
        ...
        if read_config("white_list"):
            for white_book_id in common.handle_url_list(read_config("white_list"), "syosetu"):
                book = Book()
                book.source = "syosetu"
        ...
        for page in range(self.start_page, self.end_page + 1):
            log.info(f"syosetu开始爬取第{page}页...")
            url = f"{self.domain}/?mode=favo"
            res = await request.get(url, self.header, self.session)
            if not res:
                return
            book_urls = common.get_xpath(res, "syosetu", "collection")
            if not book_urls:
                log.info("syosetu列表解析失败")
                log.debug(res)
                return
            for book_url in book_urls:
                book = Book()
                book.source = "syosetu"
                book.book_id = common.get_book_id(book_url, "syosetu")
                # 黑名单跳过
                if book.book_id in common.handle_url_list(read_config("black_list"), "syosetu"):
                    continue
                self.books.append(book)
    ...
    async def build_book_info(self, book: Book):
        ...
        res = await request.get(url, self.header, self.session)
        if not res:
            log.info(f"{url} syosetu获取书籍信息失败")
            return
        book.book_name = common.first(common.get_xpath(res, "syosetu", "title"))
        book.author = common.first(common.get_xpath(res, "syosetu", "author"))
        book.describe = common.first(common.get_xpath(res, "syosetu", "describe"))
        book.chapter_xpaths = common.get_xpath(res, "syosetu", "chapter")
        # 更新数据库
    ...
    async def build_chapter_list(self, book: Book):
        ...
        for chapter_xpath in book.chapter_xpaths:
            ...
            if not chapter_id:
                log.info(f"syosetu章节地址配置错误，跳过本章 {chapter_url}")
                continue
    ...
    async def build_content(self, chapter: Chapter):
        log.info(f"{chapter.chapter_name} syosetu开始获取章节内容...")
        url = f"{self.domain}/novel/{chapter.book_id}/{chapter.chapter_id}.html"
        text = await request.get(url, self.header, self.session)
        if not text:
            log.debug(url)
            return
        chapter.content = common.get_html(text, "syosetu", "content")
        if not chapter.content:
            log.info(f"syosetu章节无内容 {chapter.chapter_name}")
            return
        log.info(f"{chapter.chapter_name} syosetu新获取章节内容")
