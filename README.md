...
## 二改版本
以下是一些基于本项目的二次开发的版本，欢迎使用
|                           地址                           |        特性        |
|:------------------------------------------------------:|:----------------:|
| https://github.com/fisher66616/lightnovel-pydownloader | 添加了管理面板以及其余诸多的优化 |

## 适配站点
本项目设计初衷仅是用来自用，原则上只适配一手日本轻小说翻译站点  
二手转载站（如哔哩轻小说）、国轻韩轻等站点不会适配，相关请求适配的issue会直接关闭  
...
|  真白萌   | masiro |          账密登录需要使用系统安装的Chrome浏览器绕过 cf，可选择cookie登录          |
|  百合会   |  yuri  |                    一部分特殊格式的文章未适配，不建议使用                    |
| 轻小说机翻站 |  fish  |             纯自用，仅支持从收藏里直接下载epub，不接受任何相关issue              |
| hameln | hameln | 纯自用于机翻站，需挂代理，账号注册问题自行解决，仅支持cookie登录从收藏中爬取，建议开启convert_txt |

## 使用说明
### Windows
解压release最新版本的压缩包，双击lightnovel-gui.exe（带ui界面）或lightnovel.exe（原cli方式）运行
如果想直接从源码中运行，见下方Linux说明
### Linux
Linux下暂时只能从源代码运行  
安装git和python3，python需要3.11（建议3.12）  
下载源代码  
~~~bash
git clone https://github.com/ilusrdbb/lightnovel-pydownloader.git
~~~
安装requirements.txt下依赖（注意如果在windows下需要删掉uvloop这行）  
~~~bash
...
~~~
### 更新
覆盖新版本的lightnovel.exe和lightnovel-gui.exe即可
### 关于V2版本
V2版本（2.x.x）不再更新维护，建议迁移至V3版本（3.x.x），V2版本相关的issue也不再受理  
如何迁移：将V3程序（lightnovel.exe和lightnovel-gui.exe）和配置文件（config.yaml和advance.yaml，需自行重新更改），覆盖至V2版本根目录中即可
...
|     convert_txt     |                 是否同时导出txt                  |       false       |
| convert_txt_chapter |            是否同时导出txt且按照章节分割txt             |       false       |
...
|    esj_book_pwd     |          esj书籍密码配置 格式书籍或章节id: 密码           | 114514: '1919810' |
...
|    config.yaml     |              配置文件               |
|    advance.yaml    |             高级配置文件              |
|   lightnovel.exe   |           主程序（原cli方式）           |
| lightnovel-gui.exe |           主程序（带ui界面）            |
