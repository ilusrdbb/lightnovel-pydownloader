...
async def oldmasiro_get_book_list(session):
    ...
    for fid in OLD_MASIRO_FIDS:
        ...
        text = await util.http_get_text('oldmasiro', page_url, session)
...
async def oldmasiro_get_book_data(book_url, page_body, session):
    ...
    if page_body.xpath(XPATH_DICT['oldmasiro_num']):
        page_num = util.get_cost(str(page_body.xpath(XPATH_DICT['oldmasiro_num'])[0]))
    else:
        page_num = 1
    # 章节 名称对地址
    chapter_list = []
    for num in range(1, page_num + 1):
        ...
        page_text = await util.http_get_text('', page_url, session)
...
async def save_oldmasiro_book(book_data, session):
    ...
    for chapter_dict in book_data['_chapter']:
        if not chapter_dict['_index'] or not chapter_dict['_url']:
            continue
        # 处理下换行符等特殊符号
        chapter_dict['_index'][0] = util.format_text(chapter_dict['_index'][0])
        ...
        if not os.path.exists(chapter_path) or ALWAYS_UPDATE_CHAPTER:
            ...
            if '本主题需向作者支付' in text:
                if IS_PURCHASE:
                    page_body = html.fromstring(text)
                    # 获取金钱
                    cost = int(util.get_cost(page_body.xpath('//div[@class=\'locked\']//strong/text()')[0]))
                    if cost < MAX_PURCHASE:
                        formhash = page_body.xpath(XPATH_DICT['oldmasiro_formhash'])[0]
                        tid = util.get_split_str_list('&tid=', '&', chapter_dict['_url'][0])[0]
            ...
            if '抱歉，本帖要求阅读权限' in text:
                # 旧真白萌权限锁10，没办法处理
                util.write_str_data(chapter_path, '权限不足')
            else:
                page_body = html.fromstring(text)
                content_list = []
                # 只看楼主
                if page_body.xpath(XPATH_DICT['oldmasiro_follow']):
                    ...
                    if follow_text:
                        ...
                        if follow_page_body.xpath(XPATH_DICT['oldmasiro_num']):
                            page_num = util.get_cost(str(follow_page_body.xpath(XPATH_DICT['oldmasiro_num'])[0]))
                            for num in range(1, page_num + 1):
                                content_url = follow_url + '&page=' + str(num)
                                content_text = await util.http_get_text('', content_url, session)
...
async def get_dsign_text(text, session):
    ...
    new_text = await util.http_get_text('', new_url, session)
    return new_text
