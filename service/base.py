...
from service import lightnovel, login, masiro, util
from config import *
...
async def build_some_book(site_type):
    ...
    async with aiohttp.ClientSession(connector=conn, trust_env=True, cookie_jar=jar) as session:
        # 登录
        if REQUIRED_LOGIN:
            await login.start_login(site_type, session)
        # 指定线程数
...
async def build_all_book(site_type):
    ...
    async with aiohttp.ClientSession(connector=conn, trust_env=True, cookie_jar=jar) as session:
        ...
        if site_type == 'lightnovel':
            # 轻国新站属实逻辑特殊，独立出模块处理
            await lightnovel._build_lightnovel_book(session)
        else:
...
async def _async_get_book_data(book_url, site_type, session, thread_count):
    async with thread_count:
        text = await util.http_get_text('', book_url, session)
        if text:
            page_body = html.fromstring(text)
            # 轻国旧论坛 页面-只看楼主-每一楼做一章
            if site_type == 'oldlightnovel':
                await lightnovel.oldlightnovel_get_book_data(page_body, session)
            # 真白萌旧站 页面-分页循环获取章节-只看楼主全部内容做一章
            elif site_type == 'oldmasiro':
                await masiro.oldmasiro_get_book_data(book_url, page_body, session)
            # 真白萌新站 esj 页面-直接获取章节
            else:
                # 获取封面、书名、描述、章节、章节名
                book_data = {}
...
async def save_book(site_type, book_data, session):
    ...
    if not os.path.exists(cover_path) or ALWAYS_UPDATE_COVER:
        if book_data['_cover']:
            cover_res = await util.http_get_pic(URL_CONFIG.get(site_type + '_cover') % book_data['_cover'][0], session)
            if cover_res:
                util.write_byte_data(cover_path, cover_res)
    # 创建简介
    ...
    if not os.path.exists(describe_path) or ALWAYS_UPDATE_COVER:
        if book_data['_describe']:
            util.write_str_data(describe_path, ''.join(book_data['_describe']))
    # 抓取章节
...
async def save_chapter_data(site_type, chapter_data, book_path, session):
    for chapter_dict in chapter_data:
        if not chapter_dict['_index'] or not chapter_dict['_url']:
            continue
        # 处理下换行符等特殊符号
        chapter_dict['_index'][0] = util.format_text(chapter_dict['_index'][0]).lstrip()
        ...
        if not os.path.exists(chapter_path) or ALWAYS_UPDATE_CHAPTER:
            ...
            if site_type == 'masiro' and int(chapter_dict['_cost'][0]) > 0 and chapter_dict['_payed'][0] == '0':
                if IS_PURCHASE:
                    if int(chapter_dict['_cost'][0]) < MAX_PURCHASE:
                        pay_rep = await masiro.masiro_pay(chapter_dict['_cost'][0], chapter_dict['_id'][0], session)
                        if not pay_rep:
                            continue
                else:
                    continue
            ...
            if 'tieba.baidu.com' in chapter_dict['_url'][0]:
                # 贴吧有验证码，暂不考虑
                util.write_str_data(chapter_path, chapter_dict['_url'][0])
            elif 'www.ptt.cc' in chapter_dict['_url'][0]:
                text = await util.http_get_text('', chapter_dict['_url'][0], session)
                page_body = html.fromstring(text)
                # 文字内容
                ...
                text = await util.http_get_text('', URL_CONFIG[site_type + '_content'] % chapter_dict['_url'][0],
                                                session)
                page_body = html.fromstring(text)
                # 文字内容
                ...
                if len(pic_list) < 2 and LEAST_WORDS > 0 and len(content) < LEAST_WORDS:
                    ...
                    util.write_str_data(chapter_path, content)
                    # 保存插画
                    await util.save_pic_list(site_type, book_path + '/' + chapter_dict['_index'][0], pic_list, session)
